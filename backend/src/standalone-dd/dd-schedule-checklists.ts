...
export const DD_SCHEDULES: DdScheduleDefinition[] = [
  {
    ...
    items: [
      { code: "LEGAL_TITLE_SEARCH", label: "Title search & ownership chain" },
      { code: "LEGAL_COF_O", label: "Certificate of Occupancy (C of O) verification" },
      { code: "LEGAL_GOVERNORS_CONSENT", label: "Governor's Consent verification" },
      { code: "LEGAL_DEED_OF_ASSIGNMENT", label: "Deed of Assignment review" },
      { code: "LEGAL_SURVEY_CHARTING", label: "Survey plan / land charting search" },
      { code: "LEGAL_EXCISION_GAZETTE", label: "Excision & gazette verification" },
      { code: "LEGAL_ENCUMBRANCE", label: "Encumbrance / lien search" },
      { code: "LEGAL_LITIGATION", label: "Litigation & court case search" },
      { code: "LEGAL_SELLER_AUTHORITY", label: "Seller authority / power of attorney check" },
      { code: "LEGAL_LAND_USE", label: "Land-use / zoning compliance" },
    ],
  },
  {
    ...
    items: [
      { code: "ENV_FLOOD_RISK", label: "Flood risk assessment" },
      { code: "ENV_DRAINAGE", label: "Drainage & waterlogging review" },
      { code: "ENV_SOIL_TOPOGRAPHY", label: "Soil / topography concerns" },
      { code: "ENV_EROSION", label: "Erosion risk review" },
      { code: "ENV_INDUSTRIAL_PROXIMITY", label: "Industrial / land-use proximity" },
      { code: "ENV_POLLUTION", label: "Waste / pollution signals" },
      { code: "ENV_NEIGHBOURHOOD_HAZARDS", label: "Neighbourhood environmental hazards" },
    ],
  },
  {
    code: "PHYSICAL_CHECK",
    ...
    items: [
      { code: "PHYS_BOUNDARY_BEACONS", label: "Boundary / beacon verification" },
      { code: "PHYS_STRUCTURE_CONDITION", label: "Building / structure condition" },
      { code: "PHYS_ACCESS_ROW", label: "Access roads & right of way" },
      { code: "PHYS_UTILITIES", label: "Utilities (water, electricity, drainage)" },
      { code: "PHYS_MEASUREMENTS", label: "Site measurements vs survey plan" },
      { code: "PHYS_DEFECTS", label: "Visible defects / unfinished works" },
      { code: "PHYS_OCCUPANCY", label: "Occupancy status check" },
    ],
  },
  {
    code: "SECURITY_CHECK",
    ...
    items: [
      { code: "SEC_NEIGHBOURHOOD", label: "Neighbourhood crime / security posture" },
      { code: "SEC_ACCESS_CONTROL", label: "Access control & perimeter" },
      { code: "SEC_OMO_ONILE", label: "Omo-onile / local claimant risk signals" },
      { code: "SEC_DISPUTE", label: "Dispute / communal conflict signals" },
      { code: "SEC_NIGHT_SAFETY", label: "Night-time safety assessment" },
      { code: "SEC_INFRASTRUCTURE", label: "Nearby security infrastructure" },
    ],
  },
];
...
): { ok: true; scheduleCodes: DdScheduleCode[] } | { ok: false; message: string } {
  const scheduleCodes = selectedScheduleCodes(selections);
...
export function suggestedTypesForSchedules(scheduleCodes: string[]): string[] {
