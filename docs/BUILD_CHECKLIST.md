...
| 10 | DD purchase wizard, 7 steps | The wizard is complete and takes payment. The `DueDiligenceOrder` it creates **now has a lifecycle**: E1-S1 (#138) added the queue, the assignment, the report and the completion behind `dd_case_lifecycle`, which is off. What is still missing is the rest of the chain: the transaction beside the order does not reach `DD_COMPLETE` and the buyer is not offered the property purchase | E1-S2 to E1-S4 |
...
- **Last completed:** E1-S1, the listing due diligence case lifecycle, the first engineering the ADR-0001 answer released. Five routes behind `dd_case_lifecycle`: the scoped queue, the single case read, operator assignment, the assignee's report upload, and a transition table from `PAID` to `IN_PROGRESS` to `COMPLETE` that requires a verdict. It was built by extracting what the two due diligence paths share into `dd-core` rather than by copying the 1590-line standalone service, so `standalone-dd.service.ts` is 600 lines lighter. A caller who does not own a case gets 404 and not 403. The flag is off, so production behaves as it did
- **Done this session:** 40 pull requests merged across the week and the waves. DOCS-8 came before this one with the remaining-work report, CH-7 before that with rule 10 and the check behind it, and DOCS-7 before that with the ADR-0001 answer written into the record
- **Next:** **E1-S2 and E1-S3, and they can run beside each other.** Both waited on E1-S1 and on nothing else, so this is the first point in wave 4 with two startable rows. E1-S2 is the transaction state machine from `DD_PURCHASED` to `DD_COMPLETE`; E1-S3 is buyer report delivery, and E1-S1 has already made `reportStorageKeys` real on the listing path, so what is left there is the buyer's own access-checked route and the browser surface. E1-S4 follows E1-S2. Acceptance criteria are in §4 of `MVP_OUTSTANDING_BACKLOG.md` under epic E1
