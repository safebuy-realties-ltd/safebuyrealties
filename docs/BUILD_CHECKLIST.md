...
- **Done this session, 2026-08-06:** three pull requests and none of them delivers a story. #145 is the characterisation spec, test-only, carrying both Rule 8 escape hatches because no row changes state and nothing is unblocked by it. #146 committed the escalations out of a drafting buffer. The third is this one, which adds the thing that actually gets sent, `docs/escalations/ENG-DR-2026-08-06-01_Decision_Request.docx`, twelve pages with a response box under every question and a collation sheet, plus a `README.md` for the directory and a dispatch table with a column that stays empty until each item leaves. No counter on the board moves, because a question going out starts a clock rather than closing a row. **The third pull request exists because the second one lied, and it is worth naming why:** #146 left the backlog and the board saying the escalations had been sent while the escalations file itself said they were awaiting review, so three record documents disagreed about one fact and two of them were ahead of it. They now all say approved and cleared to send, and the only place a dispatch date can live is the table at the top of the escalations file, filled on the day, never from memory. **What the day cost is also worth naming:** the enumeration under EXT-8 said five conflicts and listed four in all three record documents, because the lead sentence and item four were the same conflict written twice. It now lists five and `SUR` is the fifth, kept rather than footnoted, since five items honestly enumerated beat four plus an apology
