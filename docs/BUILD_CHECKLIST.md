...
| 9 | KYC model, submission, staff review | All built, and E1-S4 (#140) made the first thing read it: `evaluatePurchaseReadiness` refuses a property purchase with `KYC_REQUIRED` when the buyer's KYC is not approved. Everywhere else it is still unread, and outside `src/kyc/` the only other backend reference is a dashboard count (`admin.service.ts:38`), so the general gate on money-moving actions is still to build | E4-S2 |
...
- **Last completed:** E1-S3, buyer due diligence report delivery, access controlled. `GET /due-diligence-orders/:id/reports` hands the owning buyer, and an operator holding `dd.orders.read`, one short-lived link per attached report. A link is the private document URL with an HMAC grant appended over the storage key, the actor id and an expiry, signed with a key derived from `JWT_SECRET` using Node's own `crypto` and nothing else, so it verifies without a table and dies inside fifteen minutes. Past the expiry the document route answers 403 rather than the file, and the signature is checked before the expiry is read so the route cannot be used as a clock or as a probe for which keys exist. The wrong buyer hears 404 because the loader throws not-found rather than forbidden, and a caller with no session hears the same 404 through `AnonymousNotFoundFilter`, which is mounted on this one handler and asserted absent from the other five. Every issued link writes an audit row naming the actor, the order and the key, which is why it is a route of its own instead of a field on the case read. The buyer screen shows availability, minutes left and the case that is complete with nothing attached, and it asks for links only when the buyer asks, so no audit row is written by a render. The flag is off, so production behaves as it did.
- **Previously:** E1-S4, the property purchase step wired to the state machine. `evaluatePurchaseReadiness` is the single rule deciding whether a buyer may purchase, and both the serializer that draws the button and the endpoint that takes the money read it, so there is no second copy for a browser to disagree with. Six refusals come back in a fixed order with a reason written for the buyer: the flag off, due diligence unfinished, a verdict of concern, KYC not passed, an escrow hold already standing, the transaction closed. `POST /payments/purchase` moves the transaction to PURCHASE_PENDING before the gateway call, a verified payment moves it to PURCHASE_IN_ESCROW and holds the full listing price rather than the deposit, and a failed or abandoned one returns it to DD_COMPLETE and leaves no escrow row. There is no amount in the request body; the server reads the price off the listing. The payment id the frontend kept in `localStorage` now travels on the transaction as `latestPayment`. The flag is off, so production behaves as it did
- **Done this session:** 43 pull requests merged across the week and the waves. E1-S4 came before this one with the property purchase step, E1-S2 before that with the transaction state machine, E1-S1 before that with the listing due diligence case lifecycle, DOCS-8 before that with the remaining-work report, CH-7 before that with rule 10 and the check behind it, and DOCS-7 before that with the ADR-0001 answer written into the record
- **Next:** **E4-S2, and it is the only row left on the schedule.** All four of E1 have merged, so nothing else is startable without a stakeholder answer first. E4-S2 is the KYC gate on money-moving actions; it listed E1-S4 as its only dependency, that has merged, and E1-S4 has already proved the pattern in one place by refusing a purchase with `KYC_REQUIRED`, so the story is generalising that to the rest of the money paths. Acceptance criteria are in §4 of `MVP_OUTSTANDING_BACKLOG.md` under epic E4
- **Blockers:** four open decisions, ADR-0002 to ADR-0005, and the external engagements EXT-1 to EXT-6. ADR-0001 was answered on 2026-08-02, which discharges half of EXT-7; the other half is ADR-0002 and it is the one to chase next, because it is what stands between the platform and a real seller being paid
