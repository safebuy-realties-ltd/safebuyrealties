...
| 10 | DD purchase wizard, 7 steps | The wizard is complete and takes payment. The `DueDiligenceOrder` it creates **now has a lifecycle, a state machine and a purchase step behind it**: E1-S1 (#138) added the queue, the assignment, the report and the completion, E1-S2 (#139) put one transition table and one enforcement point behind the transaction beside it, and E1-S4 (#140) wired the property purchase to that machine, so a completed case now leads somewhere. What is still missing is the buyer's own report delivery, an access-checked route of their own rather than the operator read | E1-S3 |
