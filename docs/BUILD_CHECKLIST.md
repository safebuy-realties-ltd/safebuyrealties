...
- **Next:** **ten of the eleven are still out, and ADR-0004 and D3 are the two that need nobody outside the company.** ADR-0004 asks somebody to accept behaviour that shipped with E3-S1, and its one live operational question is what happens to objects a storage migration cannot find; the cheapest way in is the question upstream of it, whether any real user upload ever reached a Vercel production deploy, because if none did the answer is "not applicable" and it closes in a line, and if some did then it is a loss that already happened with NDPA notification consequences rather than a migration question. D3 is manual KYC review or a provider, and only the manual branch is free, because the provider branch needs a supplier named. **Ratifying ADR-0004 releases no story**, which is worth knowing before it is treated as an unlock: E3-S2 waits on D4 **and** EXT-2, and EXT-2 survives the ratification. **EXT-9 needs a line and not a decision now**, confirmation that the collation rows at EXT-9.1 and EXT-9.3 are superseded by the response boxes, plus the two needs never answered at all. **Put the `NODE_ENV` finding on the hosting cutover checklist before the move off Vercel**, not after. The older framing follows and still holds. **The schedule is out, so what is next is recording what comes back, one item at a time.** `ENG-CS-2026-08-06-01_Closure_Schedule.docx` and its ten per-owner splits were dispatched on 2026-08-06, all eleven items, and the dispatch table at the top of `docs/escalations/2026-08-06-closure-schedule.md` carries that date against every row. **The Answer recorded column beside it is empty in all eleven, and that is the column that matters now.** Sending is not answering: nothing closed on dispatch, no story became startable, and the board on the evening of 2026-08-06 says what it said that morning. **The document is frozen.** Four revisions in place were allowed because nobody was holding a copy, eleven owners are holding one now, so a change to what the schedule asks is `ENG-CS-2026-08-06-02` authored fresh rather than a fifth revision note, and the `.docx` and the ten splits were deliberately not regenerated to carry a dispatch stamp, because what somebody is reading has to stay what was sent. **Its cover asks for a response by close of business today, 2026-08-06**, and an item that cannot be settled today comes back as `pending` with the date it is expected, which is a usable answer because work can be planned against a date and cannot be planned against silence. **The note to Digital Records went out inside the schedule rather than beside it**, because Part B is that note. EXT-8 carries the three sentences, `SUR` or `SVR` for Surveyor as a record-type segment, whether UUID really replaces `NNN` in the five identifier formats printed or approved on the same pages, and the `SBR-ALD-LOCYYXX-NNN` format written with separators and one line each on what `YY` and `XX` stand for; it carries the instruction nobody here can execute; and it asks in one line for a name and a role against the three answers relayed on 2026-08-06, because every other answer on the page names its decider. EXT-9 carries the commission rate. There is nothing left to draft for either of them. **Three of the eleven cost nobody anything outside the company:** ADR-0003, ADR-0004 and D3, two of which ask somebody to accept behaviour that already shipped while the register still reads Proposed. Settling those three takes the schedule from eleven items to eight and needs no supplier, no counsel and no signature. **EXT-3 is the largest unlock on the list that needs no judgment at all**, a sending domain and three DNS records standing in front of E6-S1 and, behind it, E6-S2 and E6-S3. **Do not re-ask anything that came back settled**, and there is a lot of it: the fourteen professional categories, both case identifier formats, the move to `SBR-SRV-TYPE`, the register split, two-sided commission, and the rule that the buyer's share is never withheld from the seller's proceeds. **Record every further reply the same way**, under the question it answers in `docs/escalations/2026-08-06-ext-8-and-ext-12.md`, with the date and the person, and fill the Answer recorded cell in the same commit. **Record a partial answer as partial**, in that word: a bare date in that column reads as closed, and three of the five answers are exactly the case that distinction exists for. **Do not resolve a contradiction toward either reading.** Where the returned document says two things, both are recorded as written and the item stays open; choosing between them is Digital Records' call and not engineering's. **E9-S2 is still held**, and by a date now rather than by a question: ID Standard Version 2 is due 2026-08-13 and nothing is encoded before it is issued, because the story writes down 32 Lagos codes and 38 national ones and the document they come from is being revised. Do not supersede any existing identifier when it lands either: that is a scoping call for Digital Records, informed by an audit nobody in this repository can run, and it is not in E9-S2's scope either way. **One instruction came back pointed the wrong way and needs answering rather than obeying:** "run the production-impact query before remediation" was addressed to us, and nobody here has production database access, which the escalation it answers says in its own text. `docs/sql/id-location-code-audit.sql` is written and ready for whoever does. The older framing still holds under it: **nothing a developer can start alone, for the second time, and this time the list got longer while it was empty.** Seventeen stories are left on the board and every one of them waits on a decision, an external party, or a story that does. The move that unblocks the most is not a decision, and it is no longer five replies either, since all five came back; it is the seven points those replies left open, and they are not evenly weighted. EXT-8 and ID Standard Version 2 stand in front of E9-S2, EXT-9 and EXT-10 in front of E9-S3, and EXT-1 in front of E2-S1, which is still what stands between this platform and a real seller being paid. **EXT-11 no longer stands in front of ADR-0004**, and that is the one genuine release of the day: the region can be chosen, with the condition that a non-Nigerian one may not carry production data until a transfer instrument is executed and documented, so the three rows behind it now wait on an artefact rather than on an answer. EXT-3, a mail domain, remains the cheapest thing on the list and holds thirteen developer-days on its own. Everything outstanding is in §4 of `MVP_OUTSTANDING_BACKLOG.md` with acceptance criteria and a citation behind each claimed gap
- **Blockers:** three decisions on the list, ADR-0003 to ADR-0005, of which only ADR-0004 holds any rows. **ADR-0004 is no longer blocked, and open is not decided.** EXT-11 closed on 2026-08-06: the NDPA 2023 and the GAID 2025 govern rather than either company document, so §12's closed list of three transfer conditions is the rule and the published policy's reasonable steps wording is not, and a second answer the same day named the mechanism the existing non-Nigerian infrastructure sits under. The region choice is therefore takeable with a condition attached, and the condition is the blocker now: a Nigerian region needs no instrument, and a non-Nigerian one may not carry production data until an NDPC-approved Cross-Border Data Transfer Instrument or standard contractual clauses are executed with the hosting provider and its subprocessors, and documented. **No such instrument exists in this repository**, so the exposure is a known gap rather than a suspected one. Twelve external engagements, EXT-1 to EXT-12, **of which nine are open**. EXT-11 and EXT-12 closed on 2026-08-06 and EXT-7 was discharged earlier; the other nine include EXT-8, EXT-9 and EXT-10, which were answered on 2026-08-06 and did not close, because an answer that contradicts itself in three places, an answer that gives the commission rate at two different numbers, and a position awaiting counsel's signature are all still blocks. **ADR-0002 is answered and EXT-7 is discharged**, but answering it released nothing: E2-S1 grew from a bank-details form into a regulated flow with reconciliation, CBN and AML duties, and its eleven developer-days moved to EXT-1 whole. There is also a second gate nobody in this repository can close: section 14.2 of the approved specification withholds production activation separately from the approval to build, so `financial_governance` stays off no matter how much of E9 is written. D3, manual KYC review or a provider, got cheaper to answer late rather than more urgent: E4-S2 shipped on manual review and reads one field through one registry, so a provider changes who writes `KycRecord.status` and not who reads it. **EXT-1 to EXT-6 are no longer blocked on nobody having written the ask down, and that is the only thing about them that changed.** The schedule saying what each needs is written and not sent, so all six still hold every day they held before: EXT-1 in front of E2-S1, EXT-3 holding thirteen developer-days on a mail domain, EXT-4 now three documents rather than two with an unanswered question about electronic execution inside the third, EXT-5 waiting on five things that are decisions rather than drafting plus a retention period for each of eight data categories, and EXT-6 needing a booking rather than a test. **ADR-0004 and ADR-0005 read Proposed in their own files**, which is why one of them is in the schedule as a ratification of behaviour already running rather than as a question. **ADR-0003 read Proposed too until 2026-08-07, when the COO accepted it as written**, and it is the first of the eleven dispatched items to close
