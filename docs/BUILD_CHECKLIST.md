...
- **Done this session, 2026-08-06:** two pull requests opened and neither of them delivers a story. One is the characterisation spec, test-only, carrying both Rule 8 escape hatches because no row changes state and nothing is unblocked by it. The other is this documentation change and the escalations it commits, which does touch all three record documents honestly. No counter on the board moves, because a question going out starts a clock rather than closing a row. **What the day cost is worth naming:** the enumeration under EXT-8 said five conflicts and listed four in all three record documents, because the lead sentence and item four were the same conflict written twice. It now lists five and `SUR` is the fifth, kept rather than footnoted, since five items honestly enumerated beat four plus an apology
