...
| 10 | DD purchase wizard, 7 steps | The wizard is complete and takes payment. The `DueDiligenceOrder` it creates **now has a lifecycle and a state machine**: E1-S1 (#138) added the queue, the assignment, the report and the completion, and E1-S2 (#139) put one transition table and one enforcement point behind the transaction beside it, so it reaches `DD_COMPLETE` on sign-off and an illegal move answers 409. Both sit behind `dd_case_lifecycle`, which is off. What is still missing is the buyer's end: they are not delivered their own report and they are not offered the property purchase | E1-S3, E1-S4 |
...
- **Last completed:** E1-S2, the transaction state machine. `TRANSACTION_TRANSITIONS` declares every legal move once and `TransactionStateService.advance` is the only place one is applied, so an illegal move answers a mapped 409 naming what would have been legal instead of a 500. A move is one conditional `updateMany` carrying its legal predecessors, so the check and the write cannot be raced apart, and the case write and the transition share a single interactive database transaction. A replayed completion writes nothing and does not notify the buyer twice, and cancelling a case no longer marks its transaction in progress. The payments, escrow, guest-checkout and standalone writers are named in the table but still write directly, which is recorded as an open gap rather than left to be found. The flag is off, so production behaves as it did
- **Done this session:** 41 pull requests merged across the week and the waves. E1-S1 came before this one with the listing due diligence case lifecycle, DOCS-8 before that with the remaining-work report, CH-7 before that with rule 10 and the check behind it, and DOCS-7 before that with the ADR-0001 answer written into the record
- **Next:** **E1-S3 and E1-S4, and they can run beside each other.** E1-S3 waited on E1-S1 alone and E1-S4 waited on E1-S2 alone, and both have merged, so this is the first point in wave 4 with two startable rows. E1-S3 is buyer report delivery, and E1-S1 has already made `reportStorageKeys` real on the listing path, so what is left there is the buyer's own access-checked route, an audit row per issued link and the browser surface. E1-S4 is the property purchase step wired to the machine E1-S2 has just built. Acceptance criteria are in §4 of `MVP_OUTSTANDING_BACKLOG.md` under epic E1
