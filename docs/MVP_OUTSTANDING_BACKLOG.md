...
| E3-S2 🔴 | Trust | Durable object storage in production | — | M | ⛔ | EXT-2 |
...
**Size** M · **Flag** none · **Deps** EXT-2 alone, since 2026-08-07. D4 is answered: `docs/adr/0006-deployment-target-and-runtime-environment.md` puts the bucket in a Nigerian region and takes the application off Vercel onto self-managed infrastructure, so the region no longer has to be argued before it can be bought. ADR-0004 already decided how a private document is served and its access half shipped as E3-S1. What is left to procure is the bucket and its credentials, and any S3-compatible provider with a Nigerian region satisfies the ADR
