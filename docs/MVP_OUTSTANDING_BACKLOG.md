...
>
> **Reconciled a ninth time on 2026-08-07.** **`D4` is answered, and the graph above has no decision left in it.** ADR-0006 puts the bucket in a Nigerian region and moves the application off Vercel onto self-managed Nigerian infrastructure, so `E3-S2` waits on EXT-2 alone: the bucket and its credentials, which is a purchase rather than an argument. `E2-S1` waits on EXT-1 alone. Both remaining boxes on the critical path are now held by an account or a bucket somebody has to open, and **for the first time in this document no open decision holds a row.** That is a smaller change than it sounds and it should be read carefully: nothing became startable, and the two rows are as stuck today as they were yesterday. What changed is who is being waited on and what the ask is, from a stakeholder who has to think to a supplier who has to be paid.
>
...
the half that was holding the region decision is the half that came back unanswered. **That cell
still reads partial as of 2026-08-07 and it should stay that way, but nothing waits on the missing
half any more**: ADR-0006 keeps the data in Nigeria, so the question of which §12 condition licenses
a transfer has no transfer to attach to. Unanswered and blocking are different states, and this is
the first item on the page to be one without the other. This paragraph
...
| EXT-11 | Legal to say which data-transfer rule governs, the internal policy's or the published one's | Client, legal | **Answered on 2026-08-06, and no longer blocking as of 2026-08-07.** The statute governs and the strict rule wins, which settles the conflict and both consent questions. The half it did not answer, which of the three §12 conditions the running infrastructure sits under, is **moot rather than outstanding**: ADR-0006 keeps the data in Nigeria, so there is no transfer and no condition to sit under. Nothing further is asked of Legal on this item. `E8-S1` still gains scope from it, see below, and the GAID 2025 is still not in `docs/inputs/` |
...
#### EXT-11, which data-transfer rule governs, and it stopped blocking on 2026-08-07
...
**Superseded in part on 2026-08-07, and the paragraphs above are left as written.** The three paragraphs before this one still hold. The one headed *What did not change is the part that was holding work* no longer describes the state of the project, and it is kept because it is a true record of where this stood on 2026-08-06 and because the reason it stopped being true is the useful part. **`docs/adr/0006-deployment-target-and-runtime-environment.md` puts object storage in a Nigerian region and moves the application off Vercel onto self-managed Nigerian infrastructure.** There is then no transfer outside Nigeria, so §12 does not engage, so none of its three conditions has to be met and none has to be recorded. D4 is answered, ADR-0004's region sub-decision is closed, and the region half of EXT-2 is discharged.

The question Legal did not answer is therefore **moot rather than outstanding**, and the difference matters for what gets asked of them next: nothing further on question 3, and the follow-up still wants question 2 restated, a copy of the GAID, and a named individual in place of "Legal". Two things about this are worth being precise about. It **complies with** the ruling rather than routing around it, because the ruling was that the statute governs and the strict rule wins, and keeping the data in Nigeria is the strictest available reading of that. And it **closes the exposure rather than only avoiding a future one**: production personal data, including the identity documents in `KycRecord`, sits on non-Nigerian infrastructure today, and this is what takes it off. Until the cutover actually runs, that exposure is live and is not discharged by a decision about where it lands.
