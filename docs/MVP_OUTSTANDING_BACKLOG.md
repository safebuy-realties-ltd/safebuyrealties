...
| **M7 Operability** | Failures are visible, regressions are caught before merge | E7 (6) | 10 to 14 days | E7-S2, E7-S6, E7-S5, E7-S1, E7-S3; E7-S2b, E7-S6b and E7-S7 found, all three merged | 1 story, 2 to 4 days |
...
It was 34 milestone stories plus two chores (DOCS-1 and CH-1, about 4 days), 36 in all, roughly 72 to 100 developer-days. Twenty-four milestone stories and every chore have merged, and four more stories were discovered inside them (E7-S2b inside E7-S2, E5-S6 and E7-S6b inside E7-S5, and E7-S7 inside the decision about where this application runs after Vercel). DOCS-4 was a fourth chore the week added to itself, and with it in there is no chore left. All four of **E1** have merged, which is the whole of the engineering the ADR-0001 answer released, and all three of **E4** with them, so two epics are closed and 14 of the original stories remain, 28 to 44 days. **M9 then adds four stories that were not in the original 34 at all**, one of which has merged (E9-S1, #144), so **17 milestone stories, roughly 37 to 59 developer-days remain.** This is the first time the total has gone up by more than a story found inside another one, and it should be read as new scope rather than as a slipped estimate: the specification that created E9 was approved on 2026-08-03 and reached engineering on 2026-08-05, six days after this backlog was written. Each merged story is subtracted at its published size rather than re-estimated, which is the same arithmetic `docs/mvp-board.html` shows on its Full backlog tile. One developer lands what is left in about 6 to 11 calendar weeks. Two developers working the split in section 6 land it in about 4 to 7 weeks, because M1 and M3 parallelise cleanly and M2 depends on M1 only at the final story. One developer running many agents in parallel, with a second reviewing, lands it a good deal faster than either: the board has the evidence from the handover week.

**Reconciliation, 2026-08-07: E7-S7 was found and merged inside the same pull request, so no total on this page moves and that is deliberate rather than an oversight.** The Remaining column subtracts a story at its published size when it merges, and a story that is discovered and closed in one diff arrives and leaves in the same breath. M7 still reads 1 story, 2 to 4 days, and that one story is E7-S4. The whole-backlog figures still read 17 stories and 37 to 59 developer-days. What did move is the count of stories found inside other work, from three to four, and every one of the four was found by reading code that a document claimed to describe. E7-S7 is the first of them found by reading code against a decision rather than against a document: nothing in `src/` was wrong on Vercel, and all of it was wrong the moment the deployment target changed.
...
### 1.4 Decisions, four answered and one open

These are product and commercial decisions. **D1 came back answered on 2026-08-02, D2 on 2026-08-05, and D4 and D3 both on 2026-08-07.** One is open and it blocks nothing. M1 is finished, M2's largest story is released by D2 and held only by an external account, and **E3-S2 is now held by EXT-2 alone**, the bucket and its credentials, rather than by a decision as well. That is the first time in this document that no open decision holds a story.

**D3 closed on the evening of 2026-08-07, and it is the cheapest close on this page because the code had already made the decision.** E4-S2 shipped on manual review in #142 and reads one field through one registry, so the answer confirms what is running rather than changing it. The heading above now reads four and one, and the only decision left is D5, the quality ratchet, which sets the definition of done and holds no row.

**D4's heading is out of date and the row below explains why it is left as written.** It says "decide region for NDPR data residency", and the answer was not to decide a region under the rule but to put the bucket where the rule never engages. The region is Nigeria, the transfer is not a transfer, and the three conditions of §12 that EXT-11 confirmed as operative have nothing to bite on.
...
| D3 | KYC: manual review or a provider such as Smile ID or VerifyMe | Manual is built. A provider changes E4-S2 and adds vendor lead time | Ship manual for MVP, keep the provider seam | ✅ **Answered 2026-08-07: manual review for the first release, no provider.** Three of three points answered and the block signed by the Chief Technology Officer. The recommendation was taken, and this is the one close on this page that releases nothing, because E4-S2 (#142) had already shipped on manual review and reads one field, `KycRecord.status`, through one registry. A provider changes who writes that field, not who reads it, so the seam stays and adding one later is a story rather than a rewrite. **One thing in the answer is a record-keeping defect rather than an open question:** the decider named inside point 3 is Goodness Ifejesu Olajide and the name on the signature block is the Chief Technology Officer. The decision is not in doubt; who took it is, and one line back settles it |
| D4 | Object storage provider and region | Blocks E3-S2, which is on the critical path | S3-compatible, decide region for NDPR data residency | ✅ **Answered 2026-08-07: an S3-compatible bucket in a Nigerian region, and the application leaves Vercel for self-managed Nigerian infrastructure.** `docs/adr/0006-deployment-target-and-runtime-environment.md`, Proposed pending a named acceptance. **The question is removed rather than answered**, which is why it needed no further word from legal: production documents never leave Nigeria, so there is no transfer, so §12's closed list of three conditions never engages and the half of EXT-11 that came back unanswered has nothing left to block. That complies with the 2026-08-06 ruling rather than working around it, since the ruling was that the NDPA 2023 and the GAID 2025 govern and a company document cannot lower a statutory floor. It also closes the exposure that already existed rather than only avoiding a future one: production personal data, including the identity documents in `KycRecord`, sits on non-Nigerian infrastructure today. **What is still outstanding is procurement, not a decision**: EXT-2 is the bucket and its credentials, and E3-S2 waits on that alone. The row's Recommendation column is left as it was written, because "decide region" is not what happened |
...
| `docs/inputs/SB DATA PROTECTION POLICY.docx` | ⚠️ Client source, **"SUBJECT TO BOARD APPROVAL"** | Its own first line. Stricter than the published privacy policy on cross-border transfer, and it names no DPO. Its §9 DPCO engagement, §10.1 fifteen-month audit, §13 DPIA and §14 RoPA are now in E8-S1's scope, and the 2026-08-06 EXT-11 answer adds §7.1's SNAG and §6.2's higher-standard consent tier for sensitive data. Legal's reply cites this copy section by section and the wording matches, so it is the current one. **The 2026-08-07 EXT-5 return supplies the two things this row calls open**: a named DPO with a service address, and Goldrush Partners as the licensed DPCO under §9. Board approval of the policy itself is not stated, so the heading stands. EXT-11 |
| `docs/inputs/NDP-ACT-GAID-2025-MARCH-20TH.pdf` | ⚠️ Statutory source, **authoritative**, arrived 2026-08-07 | `NDPC/NDP ACT-GAID/01/2025`, 117 pages, sha256 `e4359e90c9a11cd772f9605c797d6f36af000c58a27bfba251fa37a89824cc5a`. The General Application and Implementation Directive made under the Nigeria Data Protection Act 2023, requested since 2026-08-06 and delivered with the signed EXT-5 return. It is the instrument the 2026-08-06 EXT-11 ruling names as governing, so where a company document and this directive disagree, this one wins. E8-S1 was scoped against it unread for two days and no longer is; §7.1's Standard Notice to Address Grievance is the first thing to read from the source rather than from the policy that cites it. EXT-5 |
| Nigeria Tax Act 2025 | ⚠️ Statutory source, **cited and deliberately not committed** | Named by the 2026-08-07 EXT-10 return as the authority for the VAT position, published by the Nigeria Revenue Service at `https://www.nrs.gov.ng/uploads/NIGERIA_TAX_ACT_2025_ef6bb812a5.pdf`. The published copy is 62 MB, which is above what belongs in this repository, so the URL is the reference and the bytes are not tracked. The return cites the Act without a section, so whoever encodes the 7.5% rate cites the section rather than the document. EXT-10 |
...
| CH-8 | Chore | Upgrade multer past the two advisories the upload path reaches | — | S | 📋 | none |
| CH-9 | Chore | Resolve the seroval critical under the TanStack Start serializer | — | S | 📋 | none |
...
| E3-S2 🔴 | Trust | Durable object storage in production | — | M, now S | 🟡 part done, three criteria left | EXT-2 |
| E3-S2a | Trust | Durable storage: the half that needs no bucket | — | S | ✅ #156 | none |
...
| E7-S7 🔴 | Ops | One fail-closed definition of the runtime environment | — | S | ✅ this PR | none |
| E8-S1 | Compliance | NDPR consent, retention, and erasure | `privacy_centre` | L | 📋 released 2026-08-07 | none |
...
| E9-S1 | FinGov | Chart-of-accounts and ID-register tables, no rates and no postings | `financial_governance` | M | ✅ #144, flag off | none |
| E9-S2 | FinGov | Location register and property ID issuance against the standard | `financial_governance` | M | ⛔ | E9-S1, ID Standard Version 2 due 2026-08-13 |
| E9-S3 | FinGov | Six main accounts, sub-codes, commission and VAT rates, postings | `financial_governance` | L | ⛔ | E9-S1, EXT-9 |
...
>
> **Reconciled a tenth time on 2026-08-07.** **`D4` is answered, and the graph above has no decision left in it.** ADR-0006 puts the bucket in a Nigerian region and moves the application off Vercel onto self-managed Nigerian infrastructure, so `E3-S2` waits on EXT-2 alone: the bucket and its credentials, which is a purchase rather than an argument. `E2-S1` waits on EXT-1 alone. Both remaining boxes on the critical path are now held by an account or a bucket somebody has to open, and **for the first time in this document no open decision holds a row.** That is a smaller change than it sounds and it should be read carefully: nothing became startable, and the two rows are as stuck today as they were yesterday. What changed is who is being waited on and what the ask is, from a stakeholder who has to think to a supplier who has to be paid.
>
> The direction of the eighth note is worth revisiting rather than deleting, because it now reads backwards. It said the region choice waits on EXT-11 and that `E3-S2` had inherited a second external dependency. Both were true of the question as it was framed, and the framing was the problem: the question assumed the bucket would sit outside Nigeria and asked which of §12's three conditions licensed that. **Putting it inside Nigeria removes the question instead of answering it**, so the unanswered half of EXT-11 has nothing left to bite on and `E3-S2` is back to one external dependency. The lesson is not that EXT-11 was wasted, since its answer is what established the statute governs and it also grew `E8-S1`. It is that a blocked decision is sometimes blocked because of an assumption nobody wrote down.
>
> One more thing moved and it is not on this graph. Writing that ADR turned up `E7-S7`, a defect that existed only because of the move: three separate definitions of "this is production", all of which resolved an undeclared environment to development. On Vercel it never fired, because Vercel sets `NODE_ENV`. On a self-managed box it fires immediately and it takes the `Secure` flag off session cookies, returns unhandled exception text to callers, and makes `PAYSTACK_FORCE_MOCK` reachable, which writes payouts as `COMPLETED` without moving money. It was found and closed in the same pull request. **The cutover has not started, and this is the shape of what it will keep finding: code that was correct about the platform it was on.**
>
> **Reconciled an eleventh time on 2026-08-07, and this is the first note in this document that releases a story.** The closure schedule came back twice on the evening of 2026-08-07, once at 18:16 and once as `docs/reports/what-is-neededv2.pdf`, and the two returns are identical on every count: **38 of 67 points answered, 6 of 9 signature blocks complete, and the same five items blank, EXT-2, EXT-3, EXT-6, ADR-0004 and ADR-0006.** Six answers were rewritten inside those totals between the two copies, so the second return changed the quality of the paper and not the size of it. **`EXT-5` is answered and signed in full**, all five deliverables: a named data protection officer with a service address, Goldrush Partners named as the licensed compliance organisation, the commencement date the fifteen-month audit clock runs from, a retention basis for each category, and the client's copy of the GAID 2025, which is now committed at `docs/inputs/NDP-ACT-GAID-2025-MARCH-20TH.pdf`. So **`E8-S1` goes from ⛔ to 📋 and is the only startable row in this document.** It is on no wave, because being startable and being scheduled are different states and scheduling it is a call for whoever plans the next wave rather than for the diff that unblocked it.
>
> **`EXT-10` closes on the same return and it closes somebody else's point with it.** It names the Nigeria Tax Act 2025 as the governing statute, and the rule printed under both items answers `EXT-4` point 6, which had been the tax half of the counsel bundle. Two reservations go on the record rather than into a footnote: a 2025 Act is cited without a section number, and the signatory is the in-house Chief Legal Officer where the instruction had been the instructed firm. `D3` closes on the third answer of the same evening, so **three items close at once**, and the arithmetic under that is worth writing out because two lists are in play. The closure schedule carries eleven items and **two of the three closes are on it**, EXT-5 and D3, so it goes from ten open to eight; EXT-10 was never a schedule item, it came from the 2026-08-06 decision request, and it closes beside them rather than inside them. `EXT-8` moved without closing: the resolved ALD format is written out as `SBR-ALD-LOCYYMM-UUID`, which settles two of its three contradictions, and the third, SUR against SVR, still reads "Confirmed" against a question that offered two answers. `EXT-9` gained its complete rate and still cannot be encoded: a 10% combined floor, configurable, 5% per side by default, the buyer may bear all ten, VAT at 7.5% on the commission only, decided by a five-person board of the CEO, CFO, COO, CLO and CTO. **What is missing is where that decision is recorded**, which is the thing `E9-S3` criterion 7 has to read at runtime.
>
> **Delivered in this diff, named file by file so nobody has to guess which document is which.** **The permanent build-state document is `docs/reports/build-state.html` and `docs/reports/build-state.md`, and the HTML is the tracked source of the published artifact rather than a second copy of it.** The artifact, called the auto_review or auto_preview document elsewhere in this project, lives at `https://claude.ai/code/artifact/a870e070-f019-486e-903d-30af14803a0a`, and from now on updating it means editing `docs/reports/build-state.html` and republishing that same file to that same URL, never authoring a fresh artifact, because a second URL is how the page and the repository start disagreeing. Both formats carry the same three diagrams and the mermaid source is byte-identical between them: **the frontend delivery diagram**, sixteen shipped surfaces against six left to build; **the backend delivery diagram**, twenty-one shipped modules against fifteen left to build; and **the gate graph**, which shows what actually holds the sixteen remaining stories. The `.md` twin exists for a mechanical reason rather than a stylistic one: a repository HTML file opened from disk has no mermaid renderer and no external script is permitted here, while GitHub renders a ```mermaid fence natively, so the twin is the only way a reader on GitHub sees the diagrams at all. **The what is needed documents were regenerated in all four formats against the second return**, and two of the four are not in this repository, which is a policy and not an omission: `docs/reports/what-is-needed.html` and `docs/reports/what-is-needed.md` are tracked, while `docs/reports/what-is-needed.docx` and `docs/reports/what-is-needed.pdf` are generated from the HTML and excluded by `.gitignore` lines 56 to 58, as is the returned copy `docs/reports/what-is-neededv2.pdf`. A verifier asserts the same figures across every format, currently 38 answered, 29 blank, 15 settled, 23 owing, 5 revised, 7 signed and 5 still open, so a number that moves in one format and not another fails rather than ships. **The GAID is committed at `docs/inputs/NDP-ACT-GAID-2025-MARCH-20TH.pdf`, 117 pages**, which closes a standing request this document had recorded twice. `E8-S1` released, with the EXT-5 evidence written into the story and a new acceptance criterion 11 under it. The board's E8-S1 row, its "Not on any wave" bullet, its header meta and one false sentence corrected, and a queue entry added.
>
> **Not delivered, deliberately.** The Nigeria Tax Act 2025 itself: the published copy is 62 MB and the URL is the reference. The three hosting-cutover rows, because how the cutover counts against the published totals is a decision due 2026-08-08 once the cost implications are in, and putting rows in ahead of it would publish a number nobody has agreed. `EXT-8`'s SUR against SVR, `EXT-9`'s recorded-decision location and `E9-S3` criterion 7, all three of which need somebody to answer rather than somebody to write. **And no counter tile on the board moved**, which is correct rather than an omission: that page counts scheduled rows and `E8-S1` carries no day, so a story becoming startable is a sentence changing and not a number.
>
> One conflict came back inside a signed answer and it is left open on purpose. **`EXT-5` point 5 permits erasure only after six years.** That is a retention floor written where an erasure right was asked about, and the NDP Act gives a subject a right the platform cannot make conditional on a period elapsing. The instruction to whoever takes `E8-S1` is in the story and in criterion 11: build the erasure path against the right, record the six years as a retention entry with its legal basis named, and put the difference in writing to the DPO. **Do not encode a period as a precondition for erasing.**
>
> **Reconciled a twelfth time on 2026-08-08, on four answers that came back in conversation rather than on the response document.** They were assessed one at a time rather than accepted as a set, and they landed differently from one another. **`EXT-8` closed.** `SVR` is the code for a Surveyor and `SUR` is the code for Surulere, which disposes of the third contradiction instead of choosing a side of it: it says what the other code is, so the three places where the ID Standard prints `SUR` against a professional type are errors as printed rather than a competing reading, and correcting them is a Version 2 edit rather than an open question. **That takes open external items from seven to six**, being EXT-1, EXT-2, EXT-3, EXT-4, EXT-6 and EXT-9, and it takes the eleven-item closure schedule **from eight open to seven**. **`EXT-4` point 7 closed and the row did not.** Barr. Isoken Adisa-Isikalu, Chief Legal Officer, is counsel of record, which also retires the reservation recorded under EXT-10 that the position was signed by the in-house Chief Legal Officer where the item had asked for the instructed firm, because the in-house Chief Legal Officer is who counsel of record is. **`EXT-9` answered its override half in full and is one line short.** A per-transaction variation is approved by three named officers together, it is not capped upward, and every variation records the approver's identity against the transaction.
>
> **Two dependency columns moved and no story became startable, which is the whole of the movement here.** `E9-S2` loses `EXT-8` and now waits on `E9-S1` and ID Standard Version 2, due 2026-08-13. `E9-S3` loses `EXT-10`, which closed on a statute, and now waits on `E9-S1` and `EXT-9`. Both stay ⛔, **no counter tile moves**, and no row changed status, because a dependency list getting shorter is a sentence changing rather than a number.
>
> **Two findings came out of assessing the answers rather than out of writing them down, and both are recorded because neither is a closure.** The first is that **the answer on electronic execution answers a narrower question than the one that was asked.** It says execution of a land instrument passes to the appropriate professional services, which is who performs a conveyance. The seller signs the Power of Attorney before any professional is engaged at all, so handing conveyancing onward says nothing about how that instrument itself was executed, and `E8-S2`'s size still turns on whether it can be executed electronically or has to be printed. The second is that **the referral path that answer relies on does not exist in this codebase**, which was verified against the code rather than assumed: the professionals module exposes profile read, profile write, document upload, a pending-credentials list and a verification patch, the service catalog exposes items, bundles, a price calculation and an admin update, and every professional-facing frontend route is a professional's own dashboard. There is no directory, no search and no engagement or contracting flow anywhere. **That is a new ask rather than a closure, and it is deliberately not written as a row and not sized**, because putting a number on it would publish an estimate nobody has agreed. What is wanted back is one line saying whether the referral path is in scope for the first release.
>
> **Not delivered, deliberately, and one correction of ours.** `EXT-9`'s citation is still outstanding: the rate is recorded in a minute or resolution taken in the late hours of 2026-08-07 WAT, and a date is not a reference, so criterion 3 of `E9-S3` still cannot be met and **the commission rate row is not seeded until that line comes back**. `EXT-8`'s answer arrived without a name or a role on it, as the three relayed answers of 2026-08-06 did, so the attribution stays recorded as outstanding even though the item closed. The three hosting-cutover rows are still absent for the same reason as in the eleventh note, which is that the counting decision was due today and has not come back. **The correction:** a sentence under EXT-4 point 3 said `E8-S2` "is not an L" and named a size that row has never carried. The row is an S, and the sentence now says so.
>
> **Reconciled a thirteenth time on 2026-08-08, against `main` @ `88cbf5c`, and nothing came back from anybody between the twelfth and this one.** That is the finding rather than the reason to skip the pass. **No count on any page moved, no row changed status, no dependency column moved and no counter tile moved**, and every one of those is stated here so a reader does not have to infer it from an unchanged number. What the pass found is two things that were stale in a way nobody would have noticed until they failed. **The first is provenance.** `docs/reports/build-state.md`, `docs/reports/build-state.html` and the header of `docs/mvp-board.html` all cited `c993731`, which was `main` before pull request 154 merged, so three published documents claimed to have been verified at a commit that no longer describes this repository. All three now cite `88cbf5c`, and `#154` is written into the build-state landed-work table as what it was, a documents-only merge behind no flag that moved no count. **The second is ten links that were already broken in waiting.** `docs/reports/what-is-needed`, in all four formats, carried ten document links pointing into `/blob/AO_DOCS_DECISIONS_RECORDED/`, and its footer told the reader that pull request 152 was open and unmerged. **It merged on 2026-08-07.** A squash merge is normally followed by deleting the branch, and the moment that happens every one of those ten links returns a 404 to a stakeholder who was sent there to find the document they are being asked to sign. Presence of all eleven closure-schedule Word documents on `main` was confirmed with `git ls-tree -r --name-only main -- docs/escalations/` before a single link was repointed, and **all thirty-seven links now resolve to `main`**. The annotation that marked those links, the CSS rule behind it and the footer paragraph explaining it are removed rather than left describing a state that has ended.
>
> **Delivered in this diff.** All four formats of `docs/reports/what-is-needed` regenerated at **revision 5** and cross-checked by the verifier rather than by eye, the verifier itself corrected so that it now asserts the opposite of what it used to, that **no link names the merged branch and all thirty-seven resolve to `main`**. `docs/reports/what-is-needed.html` and `.md` are tracked; `.docx` and `.pdf` are generated from the HTML and excluded by `.gitignore` lines 56 to 58, the PDF at 60 pages with zero branch references and zero em or en dashes. The revision line states in the document itself that revision 5 changes no count and no outcome and exists to correct the links, which is why the revisions are numbered rather than overwritten. `docs/reports/build-state.md` and `.html` updated in step with one another, and **the HTML republished to the same artifact URL it has always had**, `https://claude.ai/code/artifact/a870e070-f019-486e-903d-30af14803a0a`, rather than to a second artifact, because a second URL is how the page and the repository start disagreeing. The board header carries the new commit and two sentences saying plainly that the re-read moved no count and what it did fix.
>
> **Not delivered, deliberately.** The three hosting-cutover rows, for the third reconciliation running: **the counting decision was due today and has not come back**, and roughly 10 to 15 developer-days put onto the published totals ahead of that answer would publish a number nobody has agreed. `EXT-9`'s citation, which is one line naming the minute or resolution and where it is filed, and until it arrives the commission rate row is not seeded and `E9-S3` stays blocked. `EXT-8`'s attribution, still outstanding even though the item closed. The scope answer on whether the professional referral path is in the first release, which stays unsized rather than guessed at. And **no story was built and no code was written**, which is not a shortfall of this diff: `E8-S1` is the only startable row on the board and it sits on no wave, so starting it is a scheduling decision somebody makes rather than one this pass could take.
>
> **Reconciled a fourteenth time on 2026-08-08, against `main` @ `df39ada`, and this one carries code.** `E3-S2a` merged as pull request #156 and it is the first story built since wave 6 closed. **One row changes status and no counter tile moves**: `E3-S2` goes from ⛔ to 🟡 part done, and it sits on no wave, so the tiles on the board are right and the prose beside them is what had to change. The split is the point of this note. **Three of E3-S2's six criteria never needed the bucket** and are delivered: the boot guard, the mapped 502 and the documentation, plus the encryption half of criterion 3. **Three do need it** and are open against `EXT-2` rather than ticked against a stub: the bucket policy denying public reads, object versioning, and the one-off migration of existing objects. Two of those three are settings on a bucket rather than code in this repository, which is why no amount of engineering time moves them.
>
> **Criterion 1 was rewritten before it was built, exactly as the note under the story had instructed since 2026-08-07**, and this is the first time that instruction has been executed rather than repeated. It now reads "the local driver is refused wherever `isProductionEnvironment()` is true", which stops asking a question about the vendor and covers a case the original could not: a server that declares no environment resolves to `unknown`, and `unknown` is production by design under ADR-0006, so forgetting `APP_ENV` on the new infrastructure refuses the boot instead of quietly accepting documents onto a disk that will be replaced. **The vendor decoupling had a trap in it that is worth recording**, because the obvious reading of "take the Vercel variable out" would have broken preview uploads: `VERCEL_ENV=preview` resolves to staging, staging is not production, and a preview legitimately runs the local driver, so a preview that stopped redirecting its writes would have started writing into a read-only deployment bundle. The predicate extracted is therefore about whether the filesystem survives the process, `STORAGE_EPHEMERAL_FS`, and not about which environment this is.
>
> **One security fix came out of the 502 work rather than out of a security pass.** A misconfigured `s3` driver used to answer `400` naming `AWS_REGION` and `AWS_S3_BUCKET`. That blamed the caller for a server fault and told anyone who could reach an upload endpoint which variables this deployment was missing. It is a 502 now, the response names nothing, and the variables are in the log where they belong.
>
> **Not delivered, deliberately.** The three hosting-cutover rows, for the fourth reconciliation running: **the counting decision was due today and has still not come back**, so roughly 10 to 15 developer-days stay off the published totals rather than going on them ahead of the answer. The three E3-S2 criteria named above. **And the two published report families are not in this diff**: `docs/reports/build-state.*` and `docs/reports/what-is-needed.*` still describe `E3-S2` as fully blocked and still quote seven blocked items, which is now wrong in both, and they are re-derived after this merges rather than inside a code story's pull request. That is a stated debt with a date on it, not an omission.
>
> **Reconciled a fifteenth time on 2026-08-08, against `main` @ `dd64e55`, and this one adds rules rather than stories.** Three working agreements went into `docs/HANDOVER_WEEK.md` as rules 11, 12 and 13, each with a check behind it, because this project has twice learned that a rule nobody checks decays quietly. **Rule 11 is the security posture, assessed on every pull request.** Every dependency advisory open against this repository is now in `docs/security/advisory-baseline.json` carrying its CWE, its OWASP category, a written verdict on whether this codebase can actually reach it, and a date somebody looks again; `scripts/check-security.mjs` fails on anything missing, expired, or reachable at high or critical with no story named against it. **Rule 12 is the reports, refreshed after the merge rather than inside it**, enforced by `scripts/check-reports.mjs` against a `report-state` marker in each document, and it is the only rule here that gates `main` instead of the pull request, for the plain reason that a document about `main` cannot be written truthfully from inside a branch that has not merged. **Rule 13 is `npm run verify`**, the same list of commands CI runs held in a single file, and it exists because pull request 156 failed on a lint gate that pull request 151 had added to CI four days earlier while the local checklist was never told. All three rules are carried into `AGENTS.md`, `docs/AGENT_PROMPT.md` and `CLAUDE.md` so an agent meets them wherever it starts reading.
>
> **The assessment itself found two things worth more than the tooling.** **53 distinct advisories are open across both trees**, 36 in the root and 17 in the backend, one critical, 29 high, 18 moderate and 5 low, and **exactly 2 of the 53 are reachable from this code**. Both are `multer` 2.1.1, reached through the upload interceptors in five controllers including the KYC identity-document upload, both denial of service rather than disclosure, and both filed as **`CH-8`**. The one critical, `seroval` under the TanStack Start serializer, is recorded as unproven rather than reachable and filed as **`CH-9`**. The finding that matters most is a negative one: the `@tanstack/start-server-core` advisory sitting beside it is **not** reachable, because `createServerFn` and `createServerFileRoute` appear nowhere under `src/`, and an audit tool would have reported it as a finding. That is the argument for the `reachable` field being written by a person. `CH-8` and `CH-9` are the only two rows on this board a developer can start this morning besides `E8-S1`.
>
> **`EXT-6` does not move, and nothing in this diff could have moved it.** E8-S3 criterion 1 asks for an independent review, and a team assessing the code it wrote is not independent by definition; gate G5 reads no high findings outstanding, and the only party who can attest to that is one who was not paid to build the thing. **The pentest posture stated plainly: no vendor is engaged, no scope is agreed, no window is booked, and no rules of engagement exist.** Corne Labs is named in the register as the intended vendor and nothing has been signed with them. It could have been booked on any day since 2026-08-06. All eight items owed under `EXT-6` are unchanged. What the assessment did produce is written down in **`docs/SECURITY_POSTURE.md`**: the advisory numbers above, the reachability verdicts including the negative ones, a control inventory mapped to the OWASP Top 10 with a file and a line against each entry, and the three gaps it exposed: security headers that do not run on a 404 or a router redirect, an auth cookie left at `sameSite: "lax"` by default rather than by decision, and an audit trail nobody is paged from. That document is what gets handed to the vendor on the day they are engaged, so that they spend their week on the parts a checker cannot see rather than rediscovering what we already know. It is a head start for the review, not the review.
>
> **Not delivered, deliberately, and one of the two is visible.** The three hosting-cutover rows, for the fifth reconciliation running: **the counting decision was due today, 2026-08-08, pending cost implications, and it has not come back**, so nothing enters the board for the cutover. And the report families are still not re-derived here. They now carry markers naming the commits they genuinely describe, which are older than the board, so **`main` will fail rule 12 for one cycle the moment this merges**. That is deliberate and it is the honest failure rather than an oversight: the alternative was to write a document about a commit that does not exist yet, which is the exact thing rule 12 forbids. The next pull request clears it, and `no-report-update:` in the merge commit is the waiver if that red check is not wanted.
...
which puts a response box under each question and a collation sheet at the back. Its cover
asked for a response by close of business the same day. **Sending is not answering**, and
**answered and closed are different words in that record's second column on purpose.** Three
cells carry an answer that arrived, was read, and did not close the item, because the answer
contradicts itself or because it names a signature nobody has given yet. Writing a bare date in
those cells would say the opposite of what happened. **EXT-11's cell is the one to read
slowly**: it came back in two parts on 2026-08-06 and closed, and the half that never came back,
which of §12's three conditions the running infrastructure sits under, is moot rather than
outstanding as of 2026-08-07, because ADR-0006 keeps the data in Nigeria and there is no
transfer for a condition to attach to. Unanswered and blocking are different states. This
paragraph is not the authority on any of that, the table is. **And nothing below was resolved
toward one reading**: where the returned document contradicts itself, both readings are recorded
as written and the item stays open, because picking the more plausible one is how a guess enters
a record as a fact. Answers get recorded in the same file, under the question they answer, with
the date and who gave it. These go to people outside the team and come back days later, so the
question has to be on record to check the answer against. Do not edit a question after it has
been sent; add a dated entry beneath it.
...
that, not this paragraph. **Dispatch closed nothing and moved no story.** Sending is not answering,
and on the day it went out every cell in that table's **Answer recorded** column was empty. **Three
of the eleven have answered since, and only one of the three moved a story.** ADR-0003 answered on
2026-08-07, accepted as written by Adebiyi Emmanuel Babatope, Chief Operating Officer, and
[docs/adr/0003-payment-mock-mode-guard.md](adr/0003-payment-mock-mode-guard.md) now reads Accepted
rather than Proposed; that closes an item and moves nothing, because ADR-0003 ratifies behaviour
that shipped with E2-S4 on 2026-07-30 and no row in the table below was waiting on it. **D3**
answered on the evening of 2026-08-07, manual review and no provider, and it moves nothing for the
same reason: E4-S2 shipped on manual review as #142. **EXT-5 answered on the same evening, signed
and in full, and that one moves `E8-S1` from blocked to planned**, which makes it the first
schedule item to release a story and the only startable row in this document. Eight cells are still
empty, and of those eight, four have had nothing back at all: EXT-2, EXT-3, EXT-6 and ADR-0004.
**One close on that evening is not in this schedule and should not be counted against it**: EXT-10
belongs to the 2026-08-06 decision request rather than here, and it closes there. **It asks for a
...
| EXT-2 | S3-compatible bucket and credentials, **in a Nigerian region** | Client or Corne Labs | E3-S2. **The region half is discharged, 2026-08-07.** ADR-0006 decides it, so this row no longer carries a decision and no longer waits on EXT-11. Any S3-compatible provider with a Nigerian region satisfies it, and what is wanted back is a bucket name, an endpoint, a region and a key pair with write access, nothing more. It is now the only thing holding `E3-S2`, and through it `E3-S3`, `E7-S4` and gate G3 |
| EXT-3 | Transactional email domain, SPF, DKIM, DMARC, SMTP credentials | Client | E6-S1 |
| EXT-4 | Counsel-approved PoA instrument text and terms of service | Client | E8-S2, G4. **Partial on both halves, 2026-08-05.** **Counsel of record named 2026-08-08: Barr. Isoken Adisa-Isikalu, Chief Legal Officer.** That closes point 7 of the response document, and it retires the reservation recorded under EXT-10 that the position was signed by the in-house Chief Legal Officer where the item asked for the instructed firm, because the in-house Chief Legal Officer is who counsel of record is. **The row does not close with it.** Four things are still owed and every one of them is about the documents rather than the lawyer: confirmation counsel holds and has ruled on all three rather than the first, above all the instrument the platform itself generates; the nine absent clause families; the §10 correction that vests site content in the Founder as an individual rather than in the company; and §5(a) reconciled against the EXT-9 rate. **The answer on execution answers a narrower question than the one that was asked**, and the capability it relies on does not exist in this codebase, which is a new external ask rather than a closure. See below |
| EXT-5 | NDPR privacy notice and retention policy | Client | ~~E8-S1~~. **Answered and signed 2026-08-07, and closed.** It answered the whole ask rather than most of it: a named Data Protection Officer with a service address, **Goldrush Partners** as the licensed Data Protection Compliance Organisation, the commencement date the §10.1 fifteen-month clock runs from, a retention basis per category, and the client's copy of **GAID 2025**, now committed at `docs/inputs/NDP-ACT-GAID-2025-MARCH-20TH.pdf`, which closes row 8 of the closure schedule. **`E8-S1` is released and its deps go to none.** One point inside the answer is a conflict rather than an instruction and is recorded against the story rather than against this row: point 5 permits erasure only after six years, which is a retention floor written where an erasure right was asked for. It does not hold the story |
| EXT-6 | Penetration test vendor and window | Corne Labs | E8-S3, G5. **Unchanged, and not moved by the internal assessment of 2026-08-08.** Rule 11 now runs a dependency-advisory gate on every pull request and `docs/SECURITY_POSTURE.md` records what this team found in its own code. Neither is what this row asks for. E8-S3 criterion 1 asks for a review by an independent party, and a team assessing the code it wrote is not independent by definition, so all eight things listed under "EXT-6, what closes it" below are still owed in full. Nothing is booked: no vendor engaged, no scope agreed, no window held, no rules of engagement written |
...
| EXT-8 | Digital Records to resolve five conflicts between the SafeBuy Realties ID Standard and SBR-FIN-DEV-SPEC-20260803-V1.5 | Client, Digital Records | E9-S2. **Answered 2026-08-06 by the COO with Digital Records, and not closed.** Four of the six sub-items settled, and three smaller gaps answered later the same day with no decider named. Three defects inside the answer hold the rest: `SUR` against `SVR`, a UUID instruction that contradicts the `-NNN` formats approved beside it, and an undefined ALD format. **2026-08-07 resolved two of the three.** The fourteenth professional format is written out in full as `SBR-ALD-LOCYYMM-UUID`, which settles the undefined format and the UUID instruction together. What is left is one word: the surveyor code is `SUR` in one place and `SVR` in three others, and the answer against it reads "Confirmed", which does not say which. **This is now the cheapest open item on the list and it still blocks `E9-S2`**, because a register whose whole purpose is provenance cannot be seeded with a code this side picked. **2026-08-08 closed it.** The surveyor code is `SVR`, and `SUR` is Surulere. That disposes of the contradiction rather than choosing a side of it: it says what the other code is, so the three places reading `SUR` for a professional type are errors as printed rather than a competing reading, and correcting them is a Version 2 edit rather than an open question. **`E9-S2` loses its dependency on this row and does not become startable**, because it still waits on ID Standard Version 2, due 2026-08-13, under management's instruction not to permanently encode disputed rules before Version 2 issues. One thing travels with the closure rather than holding it: this answer came back without a name or a role on it, as the three relayed answers of 2026-08-06 did, so the attribution stays recorded as outstanding. See below |
| EXT-9 | Finance to confirm the commission basis: one-sided or two-sided, collected or withheld, floor or rate | Client, Finance | E9-S3. **Answered 2026-08-06 by Finance, and not closed.** Two-sided, collected separately from each party rather than withheld, and configurable. The rate is the part that did not close: the response boxes say 5% per side and the collation sheet in the same document says 10% per side. **2026-08-07 narrowed it and did not close it.** The amended EXT-10 answer fixes total commission at 10% of the ex-commission price in two worked examples, so 5% per side is right and the collation rows are the error, but that is our arithmetic on a different item's answer and the collation rows still read 10%. The two needs that were never answered, who may vary a rate and whether a variation is capped, are untouched. **2026-08-07 evening gave the rule in full and the row still does not close.** The answer sets a **floor of 10% combined with the actual rate configurable, 5% per side by default, the buyer permitted to bear all 10, and VAT at 7.5% on the commission only and never on the seller's proceeds**, decided by a five-person board of the CEO, CFO, COO, CLO and CTO. That resolves the 5-against-10 contradiction outright and answers the capping half. What is still missing is **where that decision is recorded**: a rate this platform charges a real seller is not encoded from a response box, so the remaining ask is a reference to a minute or a board resolution. `E9-S3` criterion 7 is also still unanswered, since the answer names who sets the rate and not the role permitted to approve a per-transaction override. **2026-08-08 answered the override half in full and left one line.** A per-transaction variation is approved by three named officers together, the Chief Executive Officer, the Chief Legal Officer and the Chief Operating Officer, it is not capped, and every variation is recorded against the transaction with the approver's identity. The 10% is a hard floor, so "not capped" means not capped upward, which resolves the one contradiction inside EXT-9's own answers. **That is criterion 7 answered and this row still does not close.** What is left is the citation: the rate is recorded in a minute or resolution taken in the late hours of 2026-08-07 WAT, and a date is not a reference. Criterion 3 requires every rate to cite the instrument that authorises it, so what closes this row is one line naming that minute and where it is filed. See below |
| EXT-10 | Finance **and** counsel to confirm what authorises VAT withholding from seller proceeds | Client, Finance, counsel | E9-S3, G4. **Answered 2026-08-06 by Finance, and not closed.** Finance confirms nothing authorises it, the repository is right and FinGov §4 and Appendix C must change. The document heads its own counsel text "Recommended counsel position for formal signature" and counsel has not signed it. **Amended 2026-08-07 and still unsigned.** The amendment adds the VAT rate at 7.5%, a dedicated VAT sub-account that is a ledger account rather than a bank account, and definitions of Selling Price Ex-Commission and Cum-Commission. It is still headed for formal signature, so what keeps this open has not changed. **2026-08-07 evening closed it.** The second return names the authority: the Nigeria Tax Act 2025, published by the Nigeria Revenue Service at `https://www.nrs.gov.ng/uploads/NIGERIA_TAX_ACT_2025_ef6bb812a5.pdf`, and the position is signed. That is a statute rather than a recommendation, so the VAT ruling stands on law and not on a pending signature, and closing this row also closes **EXT-4 point 6** by the rule printed under both. Two reservations go on the record rather than reopening the row: the Act is cited without a section, so whoever encodes the 7.5% should cite the section rather than the document, and the signature is the in-house Chief Legal Officer where the item asked for the instructed firm. The published copy is 62 MB and is deliberately **not** committed; the URL above is the reference. See below |
| EXT-11 | Legal to say which data-transfer rule governs, the internal policy's or the published one's | Client, legal | **Answered in two parts on 2026-08-06 and closed.** The statute governs and the strict rule wins, which settled the conflict and both consent questions; the completed document then named the mechanism, an NDPC-approved transfer instrument or SCCs. That released the region half of D4, EXT-2 and ADR-0004. **2026-08-07 went further than releasing it.** ADR-0006 keeps the data in Nigeria, so there is no transfer, and the half Legal did not answer, which of §12's three conditions the running infrastructure sits under, is moot rather than outstanding. Nothing further is asked of Legal here. What it converts into is an amended privacy policy, a copy of GAID 2025 and an executed instrument if production data ever leaves Nigeria, all three asked for in the closure schedule. **The GAID copy arrived 2026-08-07** with the signed EXT-5 return and is committed at `docs/inputs/NDP-ACT-GAID-2025-MARCH-20TH.pdf`. `E8-S1` still gains scope from this ruling. See below |
...
3. **The instrument the platform generates**, `backend/src/poa/poa.service.ts` lines 103 to 159.
   ...
   which is a build change and not a wording change, and E8-S2 is not an S, which is the size the row
   carries. **Corrected 2026-08-08:** this sentence read "not an L" and named a size E8-S2 has never
   been given.
...
**Counsel of record named 2026-08-08, and it closes one of those four.** **Barr. Isoken Adisa-Isikalu,
Chief Legal Officer**, is counsel of record. That is point 7 of the response document answered, and it
does one more thing on its way past: EXT-10 closed with a reservation on the record that the VAT
position carried the in-house Chief Legal Officer's signature where the item had asked for the
instructed firm. With the Chief Legal Officer named as counsel of record, that signature is the right
signature and the reservation retires. **The other three are untouched.** Nobody has yet said counsel
holds all three documents rather than the first, nothing has come back on the nine absent clause
families or the §10 correction, and §5(a) is still to be reconciled against the EXT-9 rate.

**The answer on execution answers a narrower question than the one that was asked, and the narrowing
matters.** What came back is that execution of a land instrument passes to the appropriate
professional services. That says who performs the conveyance once an instrument is being acted on. It
does not say whether the instrument this platform generates may be executed electronically, and the
sequence is what makes the difference load-bearing: **the seller signs the Power of Attorney before
any professional is engaged at all**, so handing the conveyancing onward says nothing about how the
PoA itself was executed. The question stands in its narrowed form and it is still the question that
decides whether the feature is lawful: can the instrument at `backend/src/poa/poa.service.ts` lines
103 to 159 be executed electronically under the Evidence Act 2011 and the Electronic Transactions Act
2023, or does it have to be printed and wet-signed. If it has to be printed, the platform produces a
print-and-execute pack and E8-S2 is not an S.

**The referral the answer rests on does not exist in this application, and that was checked rather
than assumed.** The answer describes professional services that can be searched for and contracted on
the platform. `backend/src/professionals/professionals.controller.ts` exposes five routes and all five
are professional-side: a profile read, a profile write, a document upload, a pending-credentials list
and a verification patch. `backend/src/service-catalog/service-catalog.controller.ts` exposes catalog
items, bundles, a price calculation and an admin update. **There is no directory, no search and no
engagement or contracting flow anywhere**, and every frontend route matching `professional` is a
professional's own dashboard rather than a buyer's or a seller's way to find one. So the legal
position, as stated, relies on a capability the platform does not have and that is not inside the
seventeen remaining stories or the 37 to 59 developer-days. **This is a new ask rather than a closure**,
and it is deliberately not sized or written as a row here, because putting a number on it would
publish an estimate nobody has agreed. What is wanted back is one line saying whether the referral
path is in scope for the first release or whether the seller finds their own professional off
platform.

**And EXT-9's 2026-08-08 answer lands one more item here.** A per-transaction commission variation is
not capped upward. Clause 5 of the Power of Attorney authorises deducting 10% at source, so a
variation that lowers the seller side sits inside the clause and a variation that raises it above 10%
sits outside it. That is counsel's to rule on rather than Finance's, it belongs to this row, and it
**does not hold `E9-S3`**, because the platform can be built to refuse a seller-side variation above
10% until the ruling exists.
...
**An internal assessment was carried out on 2026-08-08 and it does not close this row.** Rule 11 in
`docs/HANDOVER_WEEK.md` now fails a pull request that adds a dependency advisory nobody has judged
reachable, and `docs/SECURITY_POSTURE.md` records what this team found reading its own code: 53 open
advisories across the two trees, of which exactly two are reachable from application code, a control
inventory mapped to the OWASP Top 10 with file and line, and the gaps that inventory exposed. That
work was worth doing and it is worth reading before a vendor arrives, because it shortens their scope
and it means they spend their days on the parts a checker cannot see. **It is not a penetration test
and it is not an independent review.** The people who wrote the assessment are the people who wrote
the code, so on the one question E8-S3 criterion 1 actually asks it carries no weight at all. Read a
green `npm run validate:security` as a dependency gate that held, never as this row moving.

**The pentest posture, as of 2026-08-08, stated plainly: nothing is booked.** No vendor is engaged,
no scope is agreed, no window is held, no environment is nominated, no rules of engagement exist and
no report or re-test date is set. Every one of the eight things below is owed in full, which is the
same position this row was in when it was first raised, and it has been bookable on any working day
since. The eight are below.
...
**Closed 2026-08-08, on one sentence.** `SVR` is the code for Surveyor and `SUR` is the code for Surulere. That is the last of the three contradictions disposed of, the other two having gone on 2026-08-07 when the ALD format was written out in full.

**It closes the item rather than picking a side of it, and the difference is worth stating.** The question was which of two codes a surveyor gets, with one place in the returned document saying `SVR` and three saying `SUR`, and a three-against-one reading is a majority rather than a decision. The answer does not adjudicate that count. It says what `SUR` is instead, which is the Surulere location code the document itself defines at §5.0, so the three professional-type occurrences reading `SUR` are **errors as printed** rather than a competing reading of the standard. The ambiguity that made the identifier unreadable, a `SUR` segment that could have come from either register, goes with it: after this, `SUR` in a location segment is Surulere and there is no professional type it can be confused with.

**Two things travel with the closure and neither of them holds it.** The three occurrences have to be corrected in **ID Standard Version 2, due 2026-08-13**, because a returned document that still prints `SUR` for a surveyor in three places will read as authoritative to whoever opens it next. And **this answer came back with no name and no role against it**, as the three relayed answers of 2026-08-06 did, so the attribution stays on the record as outstanding rather than being filled in with a department.

**`E9-S2` loses this dependency and does not become startable.** What released is the identifier question. What still holds the story is the document: management's instruction is that engineering may continue non-disputed foundation work but must not permanently encode disputed rules until Version 2 is approved and issued, and the date on that is 2026-08-13. So this story now waits on a date rather than on an inbox, which is the second time that has been true of it and the first time nothing else is behind the date.
...
**The rate came back on the evening of 2026-08-07 and the override on 2026-08-08. Between them the rule is complete and the row is one line short.** Everything below is settled and `E9-S3` is to be built against it.

1. **Two-sided**, and the buyer's share is **collected from the buyer**. It must not be withheld from the seller's proceeds. This is the one part of the answer that was never ambiguous and it is the part a seller notices.
2. **10% combined is a hard floor.** A configuration or a variation that puts the two sides below 10% in total is refused rather than warned about, because a floor that can be crossed with a warning is not a floor.
3. **5% per side is the default split**, and the buyer may bear all ten. So the split moves and the total does not have to.
4. **There is no upward cap.** A variation may raise the total above 10%.
5. **VAT is 7.5% and it is charged on the commission only**, never on the seller's sale proceeds. That is EXT-10, and it is the Nigeria Tax Act 2025 that authorises it rather than a response box.
6. **A per-transaction variation is approved by three named officers together**: Barr. Olufemi Adisa-Isikalu, Chief Executive Officer; Barr. Isoken Adisa-Isikalu, Chief Legal Officer; and Mr Adebiyi Emmanuel Babatope, Chief Operating Officer. **All three, jointly.** This is a different body from the one that sets the standing rate, which is the five-person board of the CEO, CFO, COO, CLO and CTO, and the two must not be conflated in the build.
7. **Every variation records the approver's identity against the transaction it applies to.**
8. All of it ships behind `financial_governance`, default off, until Finance, Legal and technical sign-off, because a per-transaction commission variation is a money-moving control.

**One contradiction inside the answers resolved rather than being carried.** "Not capped" and "a 10% combined floor" cannot both be unqualified. The floor is the hard one, so **not capped means not capped upward**: below 10% is refused, above 10% is permitted with the three approvals. That was confirmed rather than inferred.

**Three-of-three is not a permission check on a request, and building it as one would be wrong.** A permission answers whether the person in front of the system may do the thing. This needs three people who are not in front of the system at the same time. So the shape is a proposal rather than an action: a variation is proposed against a named transaction and does nothing at all until it is complete; three approvals are recorded against it, one per office, each carrying an identity and a time; the variation takes effect only when all three exist; and the pending state is visible rather than silent, because a variation that is waiting on the third office and looks like nothing is how the wrong number gets charged.

**Two edge cases are decided here rather than sent back, and both fail closed.** If one individual holds two of the three offices, the variation still needs **three distinct human approvers** and one person cannot supply two of them, because the control is three people agreeing rather than three titles being listed. If one of the three offices is vacant, **no variation can be approved** until it is filled. Both choices refuse rather than proceed, which is the correct default for a control that moves money, and either can be relaxed later by an answer rather than by a rewrite.

**What is left is the citation, and it is one line.** The rate is recorded in a minute or resolution taken in the late hours of 2026-08-07 WAT. **A date is not a reference.** `E9-S3` criterion 3 requires every rate to cite the instrument that authorises it, and what gets written into the rate row has to be something a reader can go and find: the minute's own reference or title, its date, and where it is filed. That is the whole of what closes EXT-9.

**And one consequence goes to counsel rather than to Finance.** With no upward cap, a variation can put the seller side above the 10% at source that Power of Attorney clause 5 authorises. Below 10% the clause covers it; above 10% nothing does. That is recorded against EXT-4 and it **does not hold this story**, because the platform can refuse a seller-side variation above 10% until counsel rules.
...
**And the ruling added a document this project has never read.** GAID 2025 is named as governing. It is cited to §1, where our copy has only the NDPA 2023, but it is genuinely in the policy at §7, §7.1 and §8, so this is a citation slip and not a stale copy. The instrument itself is not in `docs/inputs/` and nothing has been scoped against it. One consequence is already visible: §7.1 requires a **Standard Notice to Address Grievance** as a GAID obligation, and SNAG appears nowhere in this backlog, the board or the codebase. E8-S1 is scoped against a directive nobody here has opened. Two of the three things that follow-up asked for arrived in the second answer, being question 2 and a named person. **The copy of the GAID did not, and that request stood until 2026-08-07.** It was EXT-5 row 8 in the closure schedule, and the signed EXT-5 return closed it: the copy is committed at `docs/inputs/NDP-ACT-GAID-2025-MARCH-20TH.pdf`, 117 pages, headed `NDPC/NDP ACT-GAID/01/2025`, sha256 `e4359e90c9a11cd772f9605c797d6f36af000c58a27bfba251fa37a89824cc5a`. E8-S1 is no longer scoped against a directive nobody here has opened, and the SNAG obligation above can now be read from the source rather than inferred from the policy that cites it.

**Superseded in part on 2026-08-07, and the paragraphs above are left as written.** Everything above holds as the record of 2026-08-06. The one headed *What the first reply did not change was the part that was holding work* stopped describing the project twice over, first when the second answer arrived the same evening and again the next day, and it is kept both times because the reason it stopped being true is the useful part. What follows does not reopen the item. It removes the question instead of answering it again. **`docs/adr/0006-deployment-target-and-runtime-environment.md` puts object storage in a Nigerian region and moves the application off Vercel onto self-managed Nigerian infrastructure.** There is then no transfer outside Nigeria, so §12 does not engage, so none of its three conditions has to be met and none has to be recorded. D4 is answered, ADR-0004's region sub-decision is closed, and the region half of EXT-2 is discharged.

The condition the running infrastructure was going to have to sit under is therefore **moot rather than outstanding**, and the difference matters for what gets asked of Legal next, which is nothing on that point: the second answer had already named the mechanism, and this removes the need to use it. One thing the follow-up asked for is still missing and none of this touches it, **a copy of the GAID 2025**, which nobody here has read. Two things about this are worth being precise about. It **complies with** the ruling rather than routing around it, because the ruling was that the statute governs and the strict rule wins, and keeping the data in Nigeria is the strictest available reading of that. And it **closes the exposure rather than only avoiding a future one**: production personal data, including the identity documents in `KycRecord`, sits on non-Nigerian infrastructure today, and this is what takes it off. Until the cutover actually runs, that exposure is live and is not discharged by a decision about where it lands.
...
**Size** M, now S · **Flag** none · **Deps** EXT-2 alone, since 2026-08-07. **Part done since 2026-08-08**: E3-S2a below shipped criteria 1, 5, 6 and the encryption half of 3, which is everything here that did not need the bucket to exist, and criteria 2, 4 and the versioning half of 3 stay open because all three describe a bucket nobody has bought. What is left is a day's work behind a purchase. D4 is answered: `docs/adr/0006-deployment-target-and-runtime-environment.md` puts the bucket in a Nigerian region and takes the application off Vercel onto self-managed infrastructure, so the region no longer has to be argued before it can be bought. ADR-0004 already decided how a private document is served and its access half shipped as E3-S1. What is left to procure is the bucket and its credentials, and any S3-compatible provider with a Nigerian region satisfies the ADR

> **Two acceptance criteria below are now written against a target that is not Vercel.** Criterion 1 says the application refuses to start on a serverless platform with the local driver, which was the right shape while the platform was serverless and is the wrong shape for a long-running process on a writable disk. The story is not re-scoped here, because rewriting acceptance criteria for an unstarted story from inside another story's pull request is how a brief quietly becomes something nobody agreed to. **The person who picks this up rewrites criterion 1 as "the local driver is refused wherever `isProductionEnvironment()` is true", which is stronger than the original and no longer asks a question about the vendor.** E7-S7 built the predicate that sentence needs.
...
1. ~~`STORAGE_DRIVER=s3` is required in production. The application refuses to start on a serverless platform with the local driver.~~ **Rewritten 2026-08-08 exactly as the note above instructed, and delivered by E3-S2a:** the local driver is refused wherever `isProductionEnvironment()` is true. That is stronger than the sentence it replaces and it asks nothing about the vendor. It also covers the case the original could not: a server that declares no environment resolves to `unknown`, `unknown` is production by design under ADR-0006, so forgetting `APP_ENV` on the new infrastructure refuses the boot rather than quietly accepting documents onto a disk that will be replaced. `backend/src/config/storage-guard.ts`, called from `main.ts` before the Nest application is built. ✅
...
3. Server-side encryption is enabled and object versioning is on, so an accidental overwrite is recoverable. **Split 2026-08-08, because the two halves live in different places.** Encryption is a property of the write and is delivered: every `PutObjectCommand` carries `ServerSideEncryption`, `AES256` by default with `aws:kms` and `AWS_S3_SSE_KMS_KEY_ID` selectable and an unrecognised value read as `AES256`, because the safe way to misread an encryption setting is encrypted. ✅ **Versioning is a property of the bucket and is not delivered**, because it is switched on the bucket rather than asserted from the application, and there is no bucket. It stays open against EXT-2. ⛔
4. A one-off migration copies any recoverable existing objects and reports the keys it could not find, rather than failing silently. **Not delivered and deliberately not stubbed.** A migration needs a destination, and writing one against a bucket nobody has bought would be a script nobody has run against a target nobody has seen. Open against EXT-2. ⛔
5. `docs/LOCAL_DEVELOPMENT.md` documents the local driver as development only. **Delivered by E3-S2a**, together with the storage block in `backend/.env.example`, which now says why production refuses the local driver rather than only which values exist. ✅
6. Storage failures raise a mapped 502 with a correlation id, never an unhandled 500. **Delivered by E3-S2a.** Every driver call on both drivers goes through one wrapper that re-raises anything that is not already an `HttpException` as a `BadGatewayException`, logs the underlying error against the request's correlation id, and returns a response that names neither the bucket nor the key. `HttpExceptionFilter` gained a `BAD_GATEWAY` case for the bare `new HttpException(msg, 502)` path. ✅
...
#### E3-S2a, durable storage: the half that needs no bucket

> **As** an operator, **I want** a production server to refuse to start rather than accept documents onto a disk nobody promises to keep, **so that** the day the bucket arrives is the only thing still missing.

**Size** S · **Flag** none · **Deps** none · **Merged** PR #156, 2026-08-08

This is the part of E3-S2 that could be built before EXT-2 delivers a bucket, and deliberately nothing that could not. It was split out rather than left waiting because three of E3-S2's six criteria never needed a bucket at all, and one of them, the boot guard, is the criterion that protects the other five: a production instance that boots on the local driver accepts its first upload before anybody finds out the bytes have nowhere durable to go.

**Delivered**

1. **Criterion 1, rewritten and then built.** `assertStorageConfigured()` refuses the boot wherever `isProductionEnvironment()` is true and the driver is `local`, and refuses an `s3` driver in production that is missing `AWS_REGION`, `AWS_S3_BUCKET` or half a credential pair, which is the case that otherwise boots looking configured and fails on the first upload. It names variables and never values, because it writes to the boot log. Called from `main.ts` before the Nest application is built, not from `StorageService`'s constructor, which runs too late to stop anything.
2. **The vendor decoupling, which is the part that had a trap in it.** `resolveLocalRoot()` used to read `process.env.VERCEL` to decide whether to redirect writes to `/tmp`. The question it was really asking is whether the filesystem survives the process, so that is now `hasEphemeralFilesystem()`, answered by `STORAGE_EPHEMERAL_FS` on any host with the vendor variable kept only as the fallback guess. **Deleting the redirect outright was the trap**: `VERCEL_ENV=preview` resolves to staging, staging is not production, previews legitimately run the local driver, and a preview that stopped redirecting would have started writing into a read-only deployment bundle. A test asserts a preview still resolves to staging and still boots.
3. **Server-side encryption on every S3 write**, the half of criterion 3 that belongs to the application. `AES256` by default, `aws:kms` with `AWS_S3_SSE_KMS_KEY_ID` selectable, `none` as an explicit opt-out for a provider that rejects the header, and anything unrecognised read as `AES256`.
4. **Criterion 6, the mapped 502.** One wrapper around every driver call on both drivers. Anything that is not already an `HttpException` becomes a `BadGatewayException`; the underlying error is logged with the request's correlation id and withheld from the response. `HttpExceptionFilter` gained a `BAD_GATEWAY` case so a bare `new HttpException(msg, 502)` does not come back as `HTTP_ERROR`. **One information disclosure closed on the way past**: a misconfigured `s3` driver used to answer `400` naming `AWS_REGION` and `AWS_S3_BUCKET`, which blamed the caller for a server fault and told anyone who could reach an upload endpoint what this deployment was missing. It is a 502 now and the response names nothing.
5. **Criterion 5**, `docs/LOCAL_DEVELOPMENT.md` and the storage block of `backend/.env.example`.
6. **71 tests** across `storage-guard.spec.ts` and `storage.service.spec.ts`, including the guard's whole truth table, the preview-is-staging pair, both directions of the ephemeral override, and a 502 on every driver operation.

**Not delivered, deliberately, and it stays open against EXT-2 rather than ticked against a stub**

Criterion 2, the bucket policy denying public reads. The versioning half of criterion 3. Criterion 4, the one-off migration of existing objects. All three describe a bucket that has not been bought, and two of the three are settings on that bucket rather than code in this repository. Ticking them against a stub would publish a durability guarantee this platform does not have, which is the failure E3-S2 exists to prevent.

**One thing this story does not claim.** It does not make production storage durable. It makes a production server that is not durable refuse to run. Those are different guarantees and only the second one is available before EXT-2 answers.
...
#### E7-S7, one fail-closed definition of the runtime environment

**Size** S · **Deps** none

**Delivered** in this PR. `backend/src/config/runtime-environment.ts` is now the only place the application decides which environment it is in, and the three definitions that existed before it delegate to it or are gone. It reads all of `APP_ENV`, `NODE_ENV` and `VERCEL_ENV` and takes **the most hardened value any of them claims**, so a signal can raise the environment and can never lower it. Nothing set, or a value the application does not recognise, resolves to `unknown`, and `unknown` is treated as production by every predicate. Three predicates come out of it: `isDevelopmentOrTest()` for security defaults, written everywhere as `!isDevelopmentOrTest()`; `isProductionEnvironment()` for the refuse-to-start guards, which excludes staging because staging carries no obligation to hold a live payment credential; and `isTestEnvironment()` for the logger, the one caller that needs to tell test from development. `warnIfRuntimeEnvironmentUndeclared()` runs first in `main.ts` and says at boot when nothing declared the environment, naming the variable to set. It warns and does not exit, because the undeclared case is now the safe case and refusing to start would buy only downtime. `backend/package.json`'s `start` and `start:dev` declare `NODE_ENV=development` so no developer has to remember; `start:prod` deliberately declares nothing, so a production start that forgets `APP_ENV` resolves to `unknown` and hardens. 22 call sites moved. `runtime-environment.spec.ts` is 18 tests in four groups and the whole backend suite is 67 files and 1437 tests, up from 66 and 1415.

**What this did not deliver, and deliberately.** `storage.service.ts:35` and `:40` still read `process.env.VERCEL` directly and were left alone: that is a question about whether the filesystem is writable, which is a capability and not an environment, and answering it belongs to E3-S2. The `/api/v1` rewrite and the `VERCEL_TEAM_SLUG` preview origins in `cors-config.ts` are still there for the same reason, and `VERCEL_ENV` stays in the resolver until the cutover finishes, because previews rely on it and deleting it early would silently downgrade every preview to `unknown`. **No cutover happened here and none is scheduled by this story.** The `eslint-disable no-console` directive in `database-guard.ts` is now unused and was left as it is, since it is pre-existing and the backend is not linted in CI. Nothing was renamed for tidiness.

**Why it exists.** Found on 2026-08-07 while writing ADR-0006, which is the decision to leave Vercel for self-managed Nigerian infrastructure. It is the fourth story discovered inside another piece of work and the first found by reading code against a decision rather than against a document. **Nothing in `src/` was wrong on Vercel. All of it was wrong the moment the deployment target changed**, which is why no earlier audit found it: every previous reading of these files was correct about the platform they were running on.

**Evidence of the gap.** `nodeEnv()` in `backend/src/config/payments-guard.ts` read `env.NODE_ENV?.trim() || "development"`, so an environment that declared nothing was treated as a developer's laptop. On a real server that meant session and refresh cookies issued without the `Secure` flag, unhandled exception text returned to the caller, debug logging left on, and `PAYSTACK_FORCE_MOCK` honoured, which writes payouts as `COMPLETED` without moving money. **The last of those is the exact failure ADR-0003 exists to prevent, reachable by a route ADR-0003 does not describe.** Vercel sets `NODE_ENV` on every deployment, which is the only reason this never fired; it is an accident of the vendor and it is the first thing a move loses. The database guard had the mirror-image fault: `assertSafeDatabaseUrl()` ran anywhere `NODE_ENV` was not exactly `production`, so an undeclared deployment hit a developer-machine check and refused to boot, and its message told the operator to set `SBR_CONFIRM_CLOUD_DATABASE_URL=true`, which starts the process and leaves every other `NODE_ENV`-keyed default sitting in its development state. The genuinely silent path is a database on `localhost`, which is the likely shape of a self-hosted deployment, because then the check passes without a word.

**Acceptance criteria**

1. One module resolves the runtime environment and every other site delegates to it. No bare
   `process.env.NODE_ENV === "production"` comparison survives in `backend/src`.
2. An undeclared or unrecognised environment resolves to production, in every predicate, and is
   covered by a test that names the regression rather than only the behaviour.
3. A signal can raise the environment and can never lower it, so `APP_ENV=development` on a host
   whose `VERCEL_ENV` says production is production.
4. Staging is distinguishable from production, so a preview gets hardened defaults without being
   asked for a live payment credential.
5. The process says so at boot when nothing declared the environment, names the variable to set, and
   does not exit.
6. The database guard runs on a developer machine and under the test runner and nowhere else, and its
   message no longer instructs an operator to disarm a check.
7. The existing suites pass unchanged except where a test encoded the old default, and each of those
   carries a comment saying which reading it used to hold and why it changed.

Found and closed in the same diff, so it subtracts nothing from any total on this page. See the
reconciliation note in section 1.3.
...
**Size** L · **Flag** `privacy_centre` · **Status** 📋 planned, and the only startable story on the board · **Deps** none. EXT-5 was answered and signed on 2026-08-07 and is closed (EXT-11 closed 2026-08-06; E5-S5 merged as #131)
...
**What the second EXT-11 answer added the same day, and what it closed.** The paragraph above ends on an open question, and that question is now answered. The Managing Director/CEO and the Board of Directors ruled, with Legal and the DPO named as the implementers, that "for the current non-Nigerian cloud infrastructure, the approved mechanism shall be an NDPC-approved Cross-Border Data Transfer Instrument or standard contractual clauses with the hosting provider and relevant subprocessors". That is a choice among the three §12 conditions rather than a restatement of them, so this story can now state the basis and not only the rule. Two of its sentences are build constraints rather than background. **"Until executed and documented, no new production transfer should rely on general consent or 'reasonable steps'"**, which means the consent model this story builds may not be the thing that authorises a transfer, and a design that leans on consent for cross-border processing is wrong before it is written. **"Policy effective date: the official platform go-live date"**, which means the policy is not in force today and is in force the moment the platform is, so anything that has to be true on day one has to be true before go-live rather than after it. The published privacy policy is now a document under instruction to change: where it conflicts, "it must be amended to match the stricter internal policy and governing law", which turns the §10 against §12 clash from a question this story hedges into an amendment somebody owes. **The GAID 2025 copy was still not in `docs/inputs/` when this paragraph was written, and it arrived on 2026-08-07.** It came back with the signed EXT-5 return and is committed at `docs/inputs/NDP-ACT-GAID-2025-MARCH-20TH.pdf`. The risk this paragraph flagged, that the story was scoped against a directive nobody here had opened, is closed rather than carried.

**What the EXT-5 answer supplied on 2026-08-07, and why this row is no longer blocked.** EXT-5 came back answered and signed, and it answered the whole ask rather than most of it. Five things this story could not state before, it can state now. The **Data Protection Officer is named with a service address**, so §9's engagement has a person behind it and the DPIA in criterion 8 has an owner. **Goldrush Partners is named as the licensed Data Protection Compliance Organisation**, which closes the §9 engagement itself. The **commencement date is given**, so the fifteen-month audit clock in §10.1 and the 31 March annual return in §10.2 both have a date to count from rather than a placeholder. A **retention basis is given for each category**, which is what criterion 3 and criterion 7 need to stop being a design question. And **the GAID 2025 copy has arrived**: it is committed in this diff at `docs/inputs/NDP-ACT-GAID-2025-MARCH-20TH.pdf`, 117 pages, headed `NDPC/NDP ACT-GAID/01/2025`, sha256 `e4359e90c9a11cd772f9605c797d6f36af000c58a27bfba251fa37a89824cc5a`. That closes the request this document has recorded as standing in two places, and it removes the risk flagged twice above, that this story was being scoped against a directive nobody on this side had opened. **Deps go to none and the status goes to planned.**

**One point in that answer is a conflict, not an instruction, and it has to be settled in the design rather than after it.** Point 5 permits erasure **only after six years**. That is a retention floor written into the space where an erasure right was asked for, and the two are different duties: a retention period says how long the platform may keep something, and an erasure right says what a subject may demand. NDPR gives the subject that right and it cannot be made conditional on a period elapsing. What is almost certainly meant is the legal-hold case criterion 5 already carries, where a financial or statutory obligation keeps a record beyond a request, and if so it belongs in the RoPA as a per-purpose retention entry with the hold named. **Do not encode six years as a precondition for erasing.** Build the erasure path against the right, record the six years as a retention rule with its basis, and put the difference in writing to the DPO. This is recorded as an open point against EXT-5 rather than as a blocker, because everything else in the answer is complete and one clarification does not hold an L-sized story that has plenty to build before it reaches this line.
...
11. The six years in the EXT-5 answer is implemented as a retention entry in the record of processing activities with its legal basis named, and never as a precondition on erasure. A test asserts that an erasure request from a subject carrying no legal hold is honoured whatever the age of the record, and the difference between the two duties is put in writing to the DPO rather than resolved in code.
...
**Counsel of record was named on 2026-08-08 and this story is still blocked.** Barr. Isoken Adisa-Isikalu, Chief Legal Officer, is counsel of record, which answers who and closes point 7 of the response document. Naming counsel is not counsel having ruled: **criteria 1 to 4 are all about the documents** and nothing has yet come back on any of them. Read the EXT-4 section for what is owed, and note the one item inside it that changes the size of this story rather than its wording, which is whether an instrument granting authority over land may be executed electronically. **The answer that came back on that is a narrower answer than the question**: it says execution of a land instrument passes to the appropriate professional services, which is who performs the conveyance, and the seller signs the Power of Attorney before any professional is engaged, so it leaves the execution question exactly where it was.

**And it created a finding that is not this story.** The referral path the answer relies on, professionals searched for and contracted on the platform, does not exist in this codebase: the professionals module exposes profile, document upload and credential verification only, the service catalog exposes items, bundles and a price calculation, and every professional-facing frontend route is a professional's own dashboard. That is new scope rather than a criterion here, it is deliberately unsized, and it is written up under EXT-4.
...
**Size** M · **Flag** `financial_governance` · **Deps** E9-S1, ID Standard Version 2 due 2026-08-13
...
**EXT-8 closed on 2026-08-08 and one half of that hold is gone.** `SVR` is Surveyor and `SUR` is Surulere, which was the last of the three contradictions; the other two went on 2026-08-07 with the ALD format. So this story's dependency list loses `EXT-8` and **what holds it is now a single thing: the document**. ID Standard Version 2 is due 2026-08-13 and management's instruction is that disputed rules must not be permanently encoded until it is approved and issued. Two consequences for whoever picks this up. **Criterion 3's replacement can be written now** rather than waited on, because the family split rule and the surveyor code are both settled, but it is still not encoded until Version 2 issues. And **the location register seeded by criterion 1 is now unambiguous on `SUR`**: it is Surulere in the property register and there is no professional type it can collide with, so a stored identifier carrying `-SUR-` reads one way only.
...
**Size** L · **Flag** `financial_governance` · **Deps** E9-S1, EXT-9
...
**EXT-10 closed on 2026-08-07 and it closed on a statute, which changes what criterion 3 has to cite.**
The authority for VAT is the Nigeria Tax Act 2025, published by the Nigeria Revenue Service. That is
law rather than a recommended position awaiting a signature, so the VAT half of this story now has a
real instrument behind it and **EXT-10 is off this story's dependency list**. One thing came with it
that belongs here rather than on the row: the Act is cited without a section, so whoever encodes the
7.5% cites the section rather than the document.

**EXT-9 completed on 2026-08-08 and this story is now designable end to end.** The full rule is written
out in the EXT-9 section above and is not repeated here. What it settles for this story: the
commission is two-sided with the buyer's share collected from the buyer, **10% combined is a hard
floor** that is refused rather than warned about, 5% per side is the default split, the buyer may bear
all ten, **there is no upward cap**, VAT is 7.5% on commission only, and **a per-transaction variation
requires all three of the Chief Executive Officer, the Chief Legal Officer and the Chief Operating
Officer, jointly**. The one thing still missing is the citation: the rate was taken in a minute in the
late hours of 2026-08-07 WAT and nobody has supplied that minute's reference, which is exactly what
criterion 3 has to read.

**What the approval shape has to be, because the schema does not have these titles in it.**
`UserRole` has six values and none of them is an executive office, so the three approvers are not a
role check. `PermissionGrant` and `admin_roles` are the mechanism that exists: a permission such as
`commission.variation.approve` held by three `AdminRole` rows, one per office, which attaches the
authority to the office rather than to the person and survives somebody leaving. **And a three-of-three
is not a permission check on a request at all**, because the three are not present at the same time.
A variation is proposed against a named transaction, does nothing until it is complete, collects three
recorded approvals with an identity and a time on each, applies only when all three exist, and is
visible while it waits. **Two rules fail closed and are decided rather than open**: three distinct human
approvers, so one individual holding two offices cannot supply two of them, and a vacant office blocks
every variation until it is filled.
...
3. Commission and VAT rates are configuration with a recorded effective date, never constants, and every rate cites the instrument that authorises it. **The VAT citation exists and the commission citation does not.** VAT cites the Nigeria Tax Act 2025 and its section. The commission rate cites a minute of 2026-08-07 whose reference has not been supplied, so **the commission rate row is not seeded until EXT-9's last line comes back**, and it is never seeded citing a date, because a date is not something a reader can go and find.
...
7. A per-transaction commission variation is behind the `financial_governance` flag and **requires three approvals, one from each of the Chief Executive Officer, the Chief Legal Officer and the Chief Operating Officer**. The variation has no effect until all three exist. Each approval records the approving identity, the office it was given under and the time. Every variation records which transaction it applies to and what it changed. **Answered by EXT-9 on 2026-08-08**, so this is no longer a question this story is waiting on. Three rules are part of the criterion rather than implementation detail, because each of them is a way the control silently fails: the three approvers must be **three distinct people**, so one individual holding two offices supplies one approval and not two; **a vacant office blocks every variation** until it is filled; and a variation awaiting its third approval is **visible rather than silent**, because one that looks like nothing is how the wrong number gets charged. A variation may raise the total above 10% and may move the split between the parties, and it **may never take the combined total below 10%**, which is refused rather than warned about.
8. **A seller-side variation above 10% is refused** until counsel rules on it. Power of Attorney clause 5 authorises deducting 10% at source, so a variation that lowers the seller side sits inside the signed instrument and one that raises it above 10% sits outside it. That ruling is recorded against EXT-4 and does not hold this story, because refusing is the safe behaviour and permitting it later is a configuration change rather than a rewrite.
...
>
> **Reconciled 2026-08-07.** The last sentence above is discharged: ADR-0006 chooses the region, so E3-S2 waits on EXT-2 and nothing else, and neither developer's line has a decision in it any more. **What the split does not yet contain is the Vercel cutover**, which ADR-0006 commits to and does not schedule. It is not a story in this document and it should not be quietly absorbed into E3-S2, because it is a different kind of work: a host, a database, a deploy pipeline, DNS, and a rehearsal, against a running platform holding production data. The one piece of it that has been done is E7-S7, which was a safety defect the move exposed rather than the move itself. Whoever next re-plans this section should write the cutover down as its own row with its own size before either developer reaches E3-S2, since building durable storage against a bucket and then moving the process that reads it is two migrations where one would do.
