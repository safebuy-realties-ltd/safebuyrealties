...
size**, and its **dependencies**. It is a *sizing and sequencing* document, **not** a scoping
...
1. **Stabilize** — G1, G2, G3 (broken screens) + G4 (CI gate). *Unblocks demoability + prevents
   regressions.*
2. **Foundations** — G30 (object storage) + G32 (upload hardening), G39 (audit log), G15 (status
   lifecycle), G31 (platform config). *Everything trust-related depends on these.*
3. **Verification usability** — G20, G21, G22, G23. *Makes the staff/professional pipeline real.*
4. **DD + legal core** — G27 (catalog) → G28 (PoA) → G29 (integrity) → G26 (wizard) → G33
   (payment intents). *The product's distinctive value.*
5. **Money** — G34 (escrow ledger) → G35 (payout) → G36 (refund) → G38 (reservation). *Closes the
   transaction loop safely.*
6. **Trust-completion** — G41 (notifications), G8 (KYC), G37 (webhook hardening), G46/G47
   (rate-limit/refresh), G48 (NDPR), G50 (pentest), G49 (tests). *Launch-readiness.*
